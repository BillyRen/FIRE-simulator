...
| **E** 激进-稳健 | 0.861 | 0.873 | 0.876 | **0.860** | 0.860 ✓ |

→ **A 和 E 在所有 source 都过 0.85；B 也都过（最弱 JPN 0.883）；D 在 JPN 跌到 0.843（边际）；deprecated C 是 0.844 边际**。
...
| **D** 综合 | 0.332 | 35/54 | ≈ 0 ❌ |
| **E** 激进-稳健 | 0.518 | 31/54 | **$45,540** ✓ |

→ **A 和 E 是唯一两个 54-env 下 CEW 不崩塌的 tier**（都使用 adj=0.05；详见 §结构性发现）；B/D/deprecated-C 在 `with_cfs=True AND years ∈ {45, 60}` 极端 env 下 CEW → 0。
→ E 与 A 同为 adj=0.05 family，只在 target 上差异（0.80 vs 0.95），让 user 在 SWR 维度有 3.70% vs 2.37% 的选择。
