...
**数据源**:
- `analysis/output/guardrail_v2/baseline_agg.csv` (600 configs at target=0.85, includes all params before gating)
- `analysis/output/guardrail_v2/baseline_gated.csv` (gated subset with `gating_pass=True` column; tables §2/§3/§4 below use this 433-row subset)
**分析脚本**: `/tmp/target85_analysis.py`（探索性，未持久化）。复现：load baseline_gated.csv → filter `target==0.85 AND gating_pass==True` → groupby
...
**结论**：mr 在 target=0.85 baseline 下**实际影响 ≤ 0.0015 effFR**（worst case；typical median 0.0005）。
...
| 高 CEW amount（D 候选） | `up=0.90 lo=0.50 adj=0.15 amount mr=10` | 3.31% | 0.954 | $51,293 | $28,414 | 高 CEW，4-src 边际 |
