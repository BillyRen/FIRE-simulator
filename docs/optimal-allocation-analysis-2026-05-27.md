...
**对中国/全球居民跨周期最稳健的静态配置 = `30% Domestic Stock + 60% Global Stock + 10% Domestic Bond`**（mean rank 7.6/66，跨 164 个场景）。略偏债的 `40/50/10`（mean rank 9.0，rank std 4.0）在波动性上最稳；进入 Pareto frontier 次数最多的是 `30/70/00`（85 次）。

- 单一债券（≥80% bond）在所有场景下都是最差，100% 债券 mean rank 66/66。
- 1.2x 杠杆在 4 个 WR 档全部下挫成功率（USA 4% 下 success 87.9% → 85.2%），且只要失败路径进入最差 10% 分位，CVaR 就会塌到 0（USA 4%/4.5% 和 ALL 全部 WR 在 1.2x 下 CVaR=0；USA 3.0%/3.5% 仍有正 CVaR，但比 1.0x 大幅缩水）；**leverage=1.0 跨场景稳健最优**——与 [Memory: guardrail-allocation-leverage-sweep] 一致。
...
|---|---|---|---|---|---|---|
| **30/60/10** | **7.57** | 6.51 | 41 | 152 | 0.957 | 0.880 |
| 40/50/10 | 9.02 | **4.04** | 37 | 146 | 0.956 | 0.878 |
| 20/70/10 | 9.63 | 10.52 | 63 | 137 | 0.955 | 0.875 |
| 30/50/20 | 10.36 | 5.53 | 14 | 124 | 0.953 | 0.867 |
| 40/60/00 | 10.70 | 8.44 | 37 | 163 | 0.957 | 0.884 |
| 30/70/00 | 10.73 | 9.80 | **85** | 158 | 0.957 | 0.882 |
...
> Pareto counts here break funded_ratio ties by median_final desc, so an
> allocation only earns Pareto credit when nothing at the same FR has a
> higher median final value.
...
1.2x 杠杆在所有 wr/country 都拉低 FR。**借贷成本（CPI + 2% spread）吃掉了多余 stock 暴露的预期收益**；并且一旦失败路径进入最差 10%（USA 4% 起、ALL 全部 WR 起），尾部 CVaR 直接归零（USA 3.0%/3.5% 在 1.2x 下仍保留正 CVaR，但已较 1.0x 缩水）。这印证 [Memory: ibkr-margin-rate-vs-cpi] 的结论——只有 spread < 0 或股票预期收益显著高于借贷成本时，杠杆才能赚到钱；过去 125 年的均值情况下不存在。
