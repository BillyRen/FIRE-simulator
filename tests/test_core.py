...
# ---------------------------------------------------------------------------
# International investability calibration (source-level Global_Stock haircut)
# ---------------------------------------------------------------------------

class TestIntlCalibration:
    """Source-level investability calibration of the JST Global_Stock series.

    Academic total-return series run ~1.69 pp/yr hotter than investable
    float-adjusted indices. The wedge lives on non-US data (JST US ≈ investable
    US), so the haircut is applied per source: US contributors uncut, non-US
    contributors divided by (1 + k) before the GDP-weighted blend. Default load
    is calibrated; calibrate_intl=False returns the raw academic series.
    """

    def test_default_is_calibrated_and_additive(self):
        from simulator.data_loader import load_returns_data
        raw = load_returns_data(calibrate_intl=False)
        cal = load_returns_data(calibrate_intl=True)
        default = load_returns_data()
        # Default == calibrated.
        np.testing.assert_array_equal(default["Global_Stock"].values, cal["Global_Stock"].values)
        # Calibration changes Global_Stock but nothing else.
        assert not np.allclose(raw["Global_Stock"].values, cal["Global_Stock"].values)
        for col in ["Year", "Domestic_Stock", "Domestic_Bond", "Inflation"]:
            np.testing.assert_array_equal(raw[col].values, cal[col].values)
        # Auxiliary column is dropped from both views.
        assert "Global_Stock_calibrated" not in cal.columns
        assert "Global_Stock_calibrated" not in raw.columns
        # Same row order (sorted Country, Year) so masks align across views.
        np.testing.assert_array_equal(raw["Country"].values, cal["Country"].values)

    def test_usa_full_single_multiplicative_wedge(self):
        from simulator.data_loader import load_returns_data
        raw = load_returns_data(calibrate_intl=False)
        cal = load_returns_data(calibrate_intl=True)
        m = (raw["Country"] == "USA").values
        gross_ratio = (1.0 + cal["Global_Stock"].values[m]) / (1.0 + raw["Global_Stock"].values[m])
        # USA's basket is 100% non-US -> one constant multiplicative factor 1/(1+k).
        # Tolerance allows for the CSV's %.8f storage rounding (~1e-8).
        assert gross_ratio.std() < 1e-7
        # k ~= 1.69 pp -> factor ~= 0.9834; bounded as a genuine haircut.
        assert 0.975 < gross_ratio.mean() < 0.995

    def test_non_us_partial_haircut(self):
        from simulator.data_loader import load_returns_data
        raw = load_returns_data(calibrate_intl=False)
        cal = load_returns_data(calibrate_intl=True)
        usa_mask = (raw["Country"] == "USA").values
        usa_factor = float(((1.0 + cal["Global_Stock"].values[usa_mask])
                            / (1.0 + raw["Global_Stock"].values[usa_mask])).mean())
        # A non-US investor's Global basket contains the (uncut) US sleeve, so the
        # effective haircut is milder than USA's full wedge: ratio closer to 1.
        gbr = (raw["Country"] == "GBR").values
        gbr_ratio = ((1.0 + cal["Global_Stock"].values[gbr])
                     / (1.0 + raw["Global_Stock"].values[gbr]))
        assert gbr_ratio.mean() > usa_factor + 1e-4   # less haircut than USA
        assert gbr_ratio.mean() <= 1.0 + 1e-9         # still a haircut, never a boost

    def test_calibration_lowers_global_returns(self):
        from simulator.data_loader import load_returns_data

        def geo_real(df):
            g = df["Global_Stock"].values
            inf = df["Inflation"].values
            r = (1.0 + g) / (1.0 + inf) - 1.0
            return np.prod(1.0 + r) ** (1.0 / len(r)) - 1.0

        raw = load_returns_data(calibrate_intl=False)
        cal = load_returns_data(calibrate_intl=True)
        usa_raw = raw[raw["Country"] == "USA"]
        usa_cal = cal[cal["Country"] == "USA"]
        # USA Global drops by roughly the full wedge (~1.7 pp real geometric).
        drop = geo_real(usa_raw) - geo_real(usa_cal)
        assert 0.012 < drop < 0.022

    def test_missing_calibrated_column_fails_loud(self, tmp_path):
        from simulator.data_loader import load_returns_data
        # A CSV without Global_Stock_calibrated must NOT silently serve raw data
        # as the calibrated default — it should raise, directing to the regen step.
        csv = tmp_path / "raw_only.csv"
        pd.DataFrame({
            "Year": [1990, 1991],
            "Country": ["USA", "USA"],
            "Domestic_Stock": [0.1, 0.05],
            "Global_Stock": [0.08, 0.04],
            "Domestic_Bond": [0.03, 0.03],
            "Inflation": [0.02, 0.02],
        }).to_csv(csv, index=False)
        with pytest.raises(ValueError, match="Global_Stock_calibrated"):
            load_returns_data(str(csv), calibrate_intl=True)
        # calibrate_intl=False reads the raw file fine.
        df = load_returns_data(str(csv), calibrate_intl=False)
        assert "Global_Stock_calibrated" not in df.columns
        assert len(df) == 2
